...
/**
 * Ask Gemini to give a deeper, more comprehensive explanation of a flashcard's
 * answer, grounded in live Google Search results so it can cite real references
 * (textbooks, guidelines, review articles, etc.) instead of relying only on
 * its training data.
 */
export async function explainCardInDepth(params: {
  front: string;
  back: string;
  keyPoint?: string;
  specialty?: string;
  ...
  const { front, back, keyPoint, specialty, apiKey, model } = params;

  if (!apiKey) {
    throw new GeminiError('کلید Gemini API تنظیم نشده است. لطفاً ابتدا از بخش تنظیمات، کلید خود را وارد کنید.');
  }

  const promptText = `این یک فلش‌کارت پزشکی است که کاربر در حال مرور آن است و می‌خواهد توضیح جامع‌تری دریافت کند:

سوال/صورت کارت: ${front}
پاسخ فعلی کارت: ${back}
${keyPoint ? `نکته طلایی موجود: ${keyPoint}` : ''}
${specialty ? `تخصص/مبحث: ${specialty}` : ''}

لطفاً با استفاده از جستجوی وب، توضیح جامع‌تر، دقیق‌تر و به‌روزتری از این مبحث ارائه بده. شامل:
۱. توضیح عمیق‌تر مکانیسم/پاتوفیزیولوژی یا منطق پشت پاسخ
۲. نکات بالینی یا امتحانی مرتبط که در پاسخ فعلی نیامده
۳. در صورت وجود منابع یا گایدلاین‌های معتبر پزشکی (مثل UpToDate، Harrison's، گایدلاین‌های انجمن‌های تخصصی)، به آن‌ها اشاره کن

پاسخ را به فارسی روان، خوانا و ساختارمند (با بولت در صورت نیاز) بنویس. طولانی‌تر از حد لازم ننویس؛ روی کیفیت و دقت تمرکز کن.`;
  ...
  if (!response.ok) {
    ...
    try {
      const errData = await response.json();
      if (errData?.error?.message) {
        message = /API key/i.test(errData.error.message)
          ? 'کلید Gemini API نامعتبر است. لطفاً کلید را در تنظیمات بررسی کنید.'
